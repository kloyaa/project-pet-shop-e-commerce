...
const createProfile = async (req, res) => {
  try {
    ...
    const doesExist = await Customer.findOne({ accountId });
    if (doesExist)
...
const updateProfileVisibility = async (req, res) => {
  try {
    ...
    Customer.findOneAndUpdate({ accountId }, { visibility }, { new: true })
      .then((value) => {
        if (!value)
          return res.status(400).json({ message: "accountId not found" });
        return res.status(200).json(value);
...
const updateProfileAvatar = async (req, res) => {
  try {
    ...
    const options = {
      folder: process.env.CLOUDINARY_FOLDER + "/customer/avatar",
